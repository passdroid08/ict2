 ...
 *
 * 목표:
 * - 현재 단계(Repository/정책/프로필 완성 전)에서도 "예산/대출/월부담"을 일관되게 계산할 수 있게 만든다.
 * - 나중에 정책 영향치(loanDelta/monthlyDelta)나 금리/기간/DSR 규칙이 정해지면 input만 확장하면 된다.
 *
...
public class FinanceCalculator {
    ...
    public FinanceResult calculate(FinanceInput in) {
        ...
        if (finalLoanLimit < 0) finalLoanLimit = 0;
        ...
        BigDecimal annualRate = (input.getAnnualInterestRate() == null)
                ? new BigDecimal("0.04")
        ...
        if (finalEstimatedMonthlyPayment < 0) finalEstimatedMonthlyPayment = 0;
        ...
        return FinanceResult.builder()
                ...
                .maxAffordableNow(maxAffordableNow)
                .maxAffordableAtTarget(maxAffordableAtTarget)
                .purchaseRangeLow(rangeLow)
                .purchaseRangeHigh(rangeHigh)
                .neededLoan(neededLoan)
                .estimatedMonthlyPayment(finalEstimatedMonthlyPayment)
                .monthlyBurdenRatio(monthlyBurdenRatio)
 ...
 *
 * 핵심 아이디어:
 * - 다운페이(downPayment)를 기준으로 "대출을 어느 정도까지 허용할지"를 배수로 정합니다.
 * - 실제 금융/DSR/지역규제/소득 기반 한도는 추후 정책/룰이 확정되면 교체 가능하며,
 *   현재는 '성향에 따른 레버리지 정도'만 반영하는 단순 모델입니다.
 *
 * 배수 테이블(현재 확정):
 * - L1(매우 보수): 0.0배  -> 대출 거의 사용 안 함
 * - L2(보수)     : 1.0배
 * - L3(중립)     : 2.0배
 * - L4(공격)     : 3.0배
 * - L5(매우 공격): 4.0배  -> 최대한 대출 활용
 *
 * 예:
 * downPayment=1억이면
 * - L3 => 대출한도=2억, 구매가능=3억(다운페이+대출)
 */
	private long estimateLoanLimitByPreference(long downPayment, String loanPreference) {
	    ...
	    double multiplier;
	    switch (level) {
	        case 1: multiplier = 0.0; break;
	        case 2: multiplier = 1.0; break;
	        case 3: multiplier = 2.0; break;
	        case 4: multiplier = 3.0; break;
	        case 5: multiplier = 4.0; break;
	        default: multiplier = 2.0; // 안전장치(사실상 parse 단계에서 방지)
	    }
     ...
     *
     * - loanPreference는 반드시 "L1" ~ "L5" 문자열로 전달합니다.
     * - 의미:
     *   L1: 매우 보수(대출 거의 안 씀)
     *   L2: 보수
     *   L3: 중립
     ...
     *
     * 주의:
     * - 현재 프로젝트에서는 Enum을 쓰지 않기로 했으므로(String 유지),
     *   여기서 입력값 검증/파싱을 책임집니다.
     * - 잘못된 값이 들어오면 안전하게 L3(중립)로 처리합니다.
     */
    ...
    /**
     * "L1"~"L5"를 1~5 레벨로 변환합니다.
     * - null/빈값/알 수 없는 값 -> 3(L3: 중립)로 기본 처리
     * - 대소문자/공백은 흡수합니다. (예: " l4 " -> 4)
     */
    private int parsePreferenceLevel(String loanPreference) {
        if (loanPreference == null) return 3;

        String p = loanPreference.trim().toUpperCase();

        // 정상 케이스(권장)
        if (PREF_L1.equals(p)) return 1;
        if (PREF_L2.equals(p)) return 2;
        if (PREF_L3.equals(p)) return 3;
        if (PREF_L4.equals(p)) return 4;
        if (PREF_L5.equals(p)) return 5;

        // 예외적으로 숫자("1"~"5")가 들어오는 경우도 안전하게 흡수(디버깅/테스트 편의)
        if ("1".equals(p)) return 1;
        if ("2".equals(p)) return 2;
        if ("3".equals(p)) return 3;
        if ("4".equals(p)) return 4;
        if ("5".equals(p)) return 5;
    ...
    private long estimateAnnuityMonthlyPayment(long principal, BigDecimal annualRate, int nMonths) {
        if (principal <= 0 || nMonths <= 0) return 0L;
        ...
        if (denominator.compareTo(BigDecimal.ZERO) == 0) return 0L;
    ...
    private String judgeAssetSafety(long emergencyFund, long monthlyBudget) {
        if (monthlyBudget <= 0) {
            ...
            if (emergencyFund >= 10_000_000L) return "높음";
            if (emergencyFund >= 3_000_000L) return "보통";
            return "낮음";
        }
        ...
        if (months >= 3.0) return "높음";
        if (months >= 1.0) return "보통";
        return "낮음";
    }
    ...
    private String judgeGoalFeasibility(long targetPrice, long maxNow, long maxAtTarget) {
        ...
        if (maxAtTarget >= targetPrice) return "높음";

        // 20% 이내면 보통
        double ratio = (maxAtTarget <= 0) ? 0.0 : (double) maxAtTarget / (double) targetPrice;
        if (ratio >= 0.80) return "보통";

        // 현재 시점도 너무 낮으면 낮음
        if (maxNow < targetPrice * 0.70) return "낮음";
        return "낮음";
    }
